...
FROM (
	SELECT id_case
		...
		,rfrd_date -- , (select top 1 removal_dt from portal_redux.rptPlacement plc where plc.id_intake_fact is not null and plc.id_intake_fact=intk.id_intake_fact order by removal_dt)  removal_dt
	FROM portal_redux.tbl_intakes intk
	WHERE id_case > 0
		AND EXISTS (
			SELECT id_case
			FROM portal_redux.tbl_household_children chld
...
INSERT INTO portal_redux.rate_placement_order_specific (
	date_type
	,qry_type
	,cohort_date
	,nth_order
...
JOIN portal_redux.ref_lookup_county cnty ON cnty.county_cd BETWEEN 0
		AND 39
JOIN portal_redux.numbers nth_order ON nth_order.number BETWEEN 1
		AND 3
LEFT JOIN (
	SELECT 0 date_type
		,2 qry_type
	...
	FROM #case_placement_rankings
	JOIN portal_redux.prm_cnty cnty ON cnty.match_code = county_cd
