-- create rate_placement table

DROP TABLE IF EXISTS portal_redux.rate_placement;
CREATE TABLE portal_redux.rate_placement (
	...
	ds_trend float NULL,
	CONSTRAINT PK_rate_placement PRIMARY KEY (cohort_date,county_cd,entry_point),
	CONSTRAINT rate_placement_county_cd_FK FOREIGN KEY (county_cd) REFERENCES portal_redux.ref_lookup_county(county_cd)
);
...
truncate table portal_redux.rate_placement


insert into portal_redux.rate_placement(date_type,qry_type,cohort_date,county_cd,entry_point,cnt_households_w_plcm,cnt_referrals_u18,rate_placement)
select  0 "date_type"
			, 2 "qry_type"
			, mnth.start_date
			, refC.county_cd
...
from (select distinct [Month] start_date from portal_redux.Calendar_dim ,ref_last_dw_transfer where [Month] between '2000-01-01' and dateadd(mm,-1,cutoff_date)) mnth
join portal_redux.ref_lookup_county refC on refC.county_cd between 0 and 39
join (select distinct entry_point from portal_redux.prm_entry_point) ep on ep.entry_point=ep.entry_point
left join (
				select distinct 0 "date_type",2 "qry_type",[month] start_date
						,count(distinct plc.id_case) cnt_hh_plcm
						,cnty.cd_cnty   "county_cd"
						,ep.entry_point
				From  portal_redux.rptPlacement plc
						left join portal_redux.vw_referrals_grp ref on ref.id_intake_fact=plc.id_intake_fact
						join portal_redux.ref_last_dw_transfer dw on dw.cutoff_date=dw.cutoff_date
						join portal_redux.calendar_dim cd on cd.CALENDAR_DATE=plc.removal_dt and cd.MONTH between '2000-01-01'  and  dateadd(mm,-1,cutoff_date)
						join portal_redux.prm_cnty cnty on cnty.match_code=plc.removal_county_cd
						--if the entry point is null meaning the id_intake_fact is null in rptPlacement -- default it as a CPS referrals
						 join portal_redux.prm_entry_point ep on ep.match_code=coalesce(ref.entry_point,1)
				...
				and not exists(select * from [portal_redux].[vw_nondcfs_combine_adjacent_segments]  nd where nd.id_prsn=plc.child
				...
				and not exists(--exclude families with a child already in care
								select id_case from portal_redux.rptPlacement sic where sic.id_case=plc.id_case
...
left join portal_redux.rate_referrals_scrn_in scrn on 
...
alter table portal_redux.rate_placement  CHECK CONSTRAINT ALL;			
