DROP TABLE IF EXISTS portal_redux.ooh_wb_family_settings_cache;
CREATE TABLE [portal_redux].[ooh_wb_family_settings_cache] (
...
CREATE NONCLUSTERED INDEX [idx_ooh_wb_family_settings_cache] ON [portal_redux].[ooh_wb_family_settings_cache] (
	[age_grouping_cd]
	,[pk_gender]
	,[cd_race_census]
...
DROP TABLE IF EXISTS portal_redux.ooh_wb_family_settings_cache_query;
CREATE TABLE [portal_redux].[ooh_wb_family_settings_cache_query] (
	[age_grouping_cd] TINYINT NOT NULL, 
	[pk_gender] TINYINT NOT NULL, 
	[cd_race_census] TINYINT NOT NULL, 
	[initial_cd_placement_setting] TINYINT NOT NULL, 
...
CREATE NONCLUSTERED INDEX [idx_ooh_wb_family_settings_cache_query] ON [portal_redux].[ooh_wb_family_settings_cache_query] (
	[age_grouping_cd]
	,[pk_gender]
	,[cd_race_census]
	,[initial_cd_placement_setting]
...
BEGIN
	...
	FROM @parameters p
	INNER JOIN portal_redux.match_age_grouping_cd mag ON mag.age_grouping_cd = p.age_grouping_cd
	INNER JOIN portal_redux.match_pk_gender mg ON mg.pk_gender = p.pk_gender
	INNER JOIN portal_redux.match_cd_race_census mrc ON mrc.cd_race_census = p.cd_race_census
	INNER JOIN portal_redux.match_cd_placement_setting ips ON ips.cd_placement_setting = p.initial_cd_placement_setting
	INNER JOIN portal_redux.match_cd_placement_setting lps ON lps.cd_placement_setting = p.longest_cd_placement_setting
	INNER JOIN portal_redux.match_cd_county mc ON mc.cd_county = p.cd_county
	INNER JOIN portal_redux.match_bin_dependency_cd md ON md.bin_dependency_cd = p.bin_dependency_cd
	INNER JOIN portal_redux.match_bin_los_cd ml ON ml.bin_los_cd = p.bin_los_cd
	INNER JOIN portal_redux.match_bin_placement_cd bp ON bp.bin_placement_cd = p.bin_placement_cd
	INNER JOIN portal_redux.ref_filter_ihs_services ihs ON ihs.bin_ihs_svc_cd = p.bin_ihs_service_cd
	INNER JOIN portal_redux.ref_filter_reporter_type mrt ON mrt.cd_reporter_type = p.cd_reporter_type
	INNER JOIN portal_redux.ref_filter_access_type mat ON mat.cd_access_type = p.cd_access_type
	INNER JOIN portal_redux.match_allegation ma ON ma.cd_allegation = p.cd_allegation
	INNER JOIN portal_redux.match_finding mf ON mf.cd_finding = p.cd_finding
