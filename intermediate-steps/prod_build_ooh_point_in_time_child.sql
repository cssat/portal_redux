-- create ooh_point_in_time_child table
...
CREATE TABLE portal_redux.ooh_point_in_time_child (
	...
	pit_county_cd int NULL,
	age_grouping_cd_census int NOT NULL,
	age_grouping_cd_mix int NULL,
	dur_days int NULL,
...
begin
		...
		set @int_endDate=(select convert(varchar(8),@endDate,112));
			...
			join portal_redux.rptPlacement_Events evt on evt.id_removal_episode_fact=eps.id_removal_episode_fact 
...
--  first only
	...
	-- get point in time placement count & max_bin_los_cd 
		...
		from #kids k
		join portal_redux.rptPlacement_Events evt on evt.id_removal_episode_fact=k.id_removal_episode_fact
		...
		from portal_redux.ref_filter_nbr_placement q 
		...
		from #kids kids
		join portal_redux.ref_filter_los los on kids.dur_days between los.dur_days_from and  los.dur_days_thru
		...
		and q.id_removal_episode_fact=kids.id_removal_episode_fact
			...
			from #kids k
			join portal_redux.age_dim ad on ad.age_mo=portal_redux.fnc_datediff_mos(birth_dt,point_in_time_date)		
		...
		from portal_redux.rptPlacement_Events pe
		...
		set int_match_param_key_census=	 cast(power(10.0,8) + 
			...
			--  select eps.*
			 from #kids eps
			 join  portal_redux.rptPlacement_events tcps
	  ...
	  ,row_count=(select count(*) from portal_redux.ooh_point_in_time_child)
		where tbl_id=46
