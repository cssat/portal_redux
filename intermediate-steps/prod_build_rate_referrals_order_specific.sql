-- create rate_referrals_order_specific table

DROP TABLE IF EXISTS portal_redux.rate_referrals_order_specific;
CREATE TABLE portal_redux.rate_referrals_order_specific (
	...
	CONSTRAINT PK_rate_referrals_order_specific PRIMARY KEY (nth_order,start_date,date_type,qry_type,county_cd),
...
from 	 #referrals   curr  -- all referrals
    ...
    where exists(select intake_grouper from  portal_redux.tbl_household_children chld 
...
alter table portal_redux.rate_referrals_order_specific NOCHECK CONSTRAINT ALL

truncate table portal_redux.rate_referrals_order_specific

insert into portal_redux.rate_referrals_order_specific(nth_order,[start_date],date_type,qry_type,county_cd,cnt_referrals,prior_order_cnt_households,referral_rate)
select distinct
...
from  (select distinct [month] from portal_redux.calendar_dim 
 ...
 join (select number nth_order from portal_redux.numbers ) n on n.nth_order between 1 and 100
 left join (-- this  gives the count of nth order referrals
					...
					from  #referrals ref 
					join portal_redux.prm_cnty cnty on ref.intake_county_cd=cnty.match_code
