-- create rate_care_day_maltreatment table

DROP TABLE IF EXISTS portal_redux.rate_care_day_maltreatment;
CREATE TABLE portal_redux.rate_care_day_maltreatment (
	date_type int NULL,
	qry_type int NULL,
	...
	CONSTRAINT PK__rate_car__12D2EEB87BE1AC60 PRIMARY KEY (county_cd,fiscal_yr)
);
...
insert into  portal_redux.rate_care_day_maltreatment(date_type,qry_type,fiscal_yr,county_cd
	...
	select  0 date_type,2 qry_type
	 ...
	 from portal_redux.placement_care_days_mobility  cd
	 left join (select distinct 0 date_type
					...
					from   (
									select rp.id_case,rp.child,rp.removal_dt,rp.id_removal_episode_fact
											...
											,rp.end_date
											,plc.[18bday]
								from portal_redux.rptPlacement_Events rp 
								join portal_redux.rptPlacement plc on plc.id_removal_episode_fact=rp.id_removal_episode_fact
								join portal_redux.ref_last_dw_transfer dw on dw.cutoff_date=dw.cutoff_date
								join portal_redux.prm_cnty cnty on cnty.match_code=rp.derived_county -- county location of child placement
								where  rp.begin_date<plc.[18bday] -- child is less than 18
										and plc.max_bin_los_cd>=1 -- episode at least 8 days long
								) q 
							join portal_redux.INTAKE_VICTIM_FACT ivf  on ivf.ID_PRSN_VCTM=q.child
							join portal_redux.vw_referrals_grp intk on intk.ID_INTAKE_FACT=ivf.ID_INTAKE_FACT
							...
							where  not exists (select id_prsn from portal_redux.vw_nondcfs_combine_adjacent_segments nd where nd.id_prsn=ivf.ID_PRSN_VCTM 
										and convert(datetime,convert(varchar(10),isnull(grp_rfrd_date,rfrd_date),121))
												between nd.cust_begin and nd.cust_end)
								and not exists(select evt.ID_REMOVAL_EPISODE_FACT from portal_redux.WRK_TRHEvents evt where  evt.id_removal_episode_fact= q.id_removal_episode_fact
										and  convert(datetime,convert(varchar(10),isnull(grp_rfrd_date,rfrd_date),121))
