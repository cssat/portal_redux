...
-- create tbl_ihs_services table

DROP TABLE IF EXISTS portal_redux.tbl_ihs_services;
CREATE TABLE CA_ODS.portal_redux.tbl_ihs_services (
	...
	fl_no_pay int NULL,
	CONSTRAINT PK__tbl_ihs___FC12E0DD9F0E6032 PRIMARY KEY (id_ihs_episode,dtl_id_payment_fact)
);
...
-- populate both tbl_ihs_episodes & tbl_ihs_services tables
...
begin
	...
	SELECT HM.* ,DD.CD_INVS_DISP,DD.tx_INVS_DISP  
	 into #cps
	 FROM #ihs_intk HM
	 JOIN portal_redux.TBL_INTAKES IVF ON IVF.ID_INTAKE_FACT=HM.ID_INTAKE_FACT
	 JOIN portal_redux.INVESTIGATION_ASSESSMENT_FACT iaf ON IAF.ID_INVESTIGATION_ASSESSMENT_FACT=IVF.ID_INVESTIGATION_ASSESSMENT_FACT
	 JOIN portal_redux.DISPOSITION_DIM DD ON DD.ID_DISPOSITION_DIM=IAF.ID_DISPOSITION_DIM
...
-- start retrieving all assignments that are either designated 'out-of-home care' or cps for social workers
	...
	SELECT  
		...
		, portal_redux.IntDate_to_CalDate(af.ID_CALENDAR_DIM_BEGIN) as ihs_begin_date
		, max(case when isnull(portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_END),'12/31/3999') > coalesce(dt_case_cls,'12/31/3999')
					then dt_case_cls
		  else isnull(portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_END),'12/31/3999') 
		  end )as ihs_end_date
		...
		, datediff(dd,inv_ass_start,portal_redux.IntDate_to_CalDate(af.ID_CALENDAR_DIM_BEGIN)) as days_from_rfrd_date
	...
	into #ih_assgn_all
	FROM #ihs_intk inf 
 		join portal_redux.ASSIGNMENT_FACT af on inf.id_case=af.id_case
			AND INF.INV_ASS_START <=portal_redux.IntDate_to_CalDate(af.ID_CALENDAR_DIM_BEGIN)
		join portal_redux.ASSIGNMENT_ATTRIBUTE_DIM aad on af.ID_ASSIGNMENT_ATTRIBUTE_DIM=aad.ID_ASSIGNMENT_ATTRIBUTE_DIM
		join portal_redux.worker_dim wd on wd.id_worker_dim=af.id_worker_dim and wd.CD_JOB_CLS in (9,10,11)
		left join portal_redux.TBL_case_dim cd on cd.id_case=af.id_case
				and portal_redux.IntDate_to_CalDate(af.ID_CALENDAR_DIM_BEGIN)<dt_case_cls
				and coalesce(portal_redux.IntDate_to_CalDate(af.ID_CALENDAR_DIM_END),'12/31/3999')>dt_case_opn
		WHERE  inf.id_case>0 and portal_redux.IntDate_to_CalDate(af.ID_CALENDAR_DIM_BEGIN) >= inf.INV_ASS_START and 
				portal_redux.IntDate_to_CalDate(af.ID_CALENDAR_DIM_BEGIN) < isnull(inf.case_nxt_intake,'12/31/3999')
			and (
					(
					aad.CD_ASGN_RSPNS=7 
					...
					and	(
							(
								aad.CD_ASGN_ROLE =2 
								and  isnull(portal_redux.IntDate_to_CalDate(af.ID_CALENDAR_DIM_END),cast('12/31/3999' as datetime))
											 > @start_date 
								and  isnull(portal_redux.IntDate_to_CalDate(af.ID_CALENDAR_DIM_END),cast('12/31/3999' as datetime)) 
		...
		group by inf.id_intake_fact,af.ID_CASE,portal_redux.IntDate_to_CalDate(af.ID_CALENDAR_DIM_BEGIN) ,aad.cd_asgn_type
	  ...
	  -- BEGIN DELETE THOSE WITH OVERLAPPING OUT OF HOME CARE ------------------------------------------------------
		...
		--select id_prsn_child,ihs.ihs_begin_date,ihs.ihs_end_date,tce.state_custody_start_date,tce.federal_discharge_date
		from #ih_assgn_all ihs  
		join portal_redux.tbl_child_episodes tce on tce.id_case=ihs.id_case
		...
		--select id_prsn_child,ihs.ihs_begin_date,ihs.ihs_end_date,tce.state_custody_start_date,tce.federal_discharge_date
		from #ih_assgn_all ihs  
		join portal_redux.tbl_child_episodes tce on tce.id_intake_fact=ihs.id_intake_fact
		and isnull(tce.federal_discharge_date,'12/31/3999') >=ihs.ihs_begin_date 
		and tce.state_custody_start_date < ihs.ihs_end_date
		...
		select 
			...
			from (
				...
				(select id_case,v.[number],t.ihs_begin_date,t.ihs_end_date,DATEDIFF(d, ihs_begin_date, ihs_end_date) as days_between,
					...
					from   @t t
					inner join portal_redux.numbers v
...
/****************************************************   get all in-home PAYMENT by  service begin date  ***********/		
			...
			select distinct
				...
				,af.ID_PRSN_CHILD
				,af.CHILD_AGE
				,portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_SERVICE_BEGIN) as [srvc_dt_begin]
				,portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_SERVICE_END) as [srvc_dt_end]
				,sc.cd_srvc
				,sc.tx_srvc
				,af.am_rate
				...
				,inv_ass_start
				,case_nxt_intake
				, datediff(dd,inv_ass_start,portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_SERVICE_BEGIN)) as days_from_rfrd_date 
				,intake_county_cd
				,intake_zip
			into #ihs_pay_srvc_dtl 
			from portal_redux.payment_fact af
			join #ihs_intk intk on intk.id_case=af.id_case
				and intk.INV_ASS_START <=portal_redux.IntDate_to_CalDate(af.ID_CALENDAR_DIM_SERVICE_BEGIN )
			join  portal_redux.SERVICE_TYPE_DIM std on std.ID_SERVICE_TYPE_DIM =af.ID_SERVICE_TYPE_DIM
			join portal_redux.ref_service_category sc on sc.cd_srvc=std.cd_srvc and sc.fl_ihs_svc=1
			WHERE isnull(portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_SERVICE_END),'12/31/3999') >= @start_date -- '1997-01-01'-- 
			and coalesce(portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_SERVICE_END),'12/31/3999') < isnull(intk.case_nxt_intake,'12/31/3999')
			and  af.am_total_paid <>0
			and (af.id_case is not null or af.id_case<>0)
			group by af.id_payment_fact,af.id_case,portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_SERVICE_BEGIN)
			,portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_SERVICE_END),sc.cd_srvc,sc.tx_srvc
			,af.am_rate,af.id_service_type_dim
				...
				,inv_ass_start
				,case_nxt_intake
				, datediff(dd,inv_ass_start,portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_SERVICE_BEGIN))
				,af.ID_PRSN_CHILD
				,af.CHILD_AGE
				,intake_county_cd
				,intake_zip
...
/***************************************************  FINAL TABLE **************************************************************/
		...
		if object_ID(N'portal_redux.tbl_ihs_episodes',N'U') is not null  truncate table portal_redux.tbl_ihs_episodes;
insert into portal_redux.tbl_ihs_episodes(
					id_ihs_episode
					,id_case
					,case_sort
					,ihs_begin_date
...
if object_ID(N'portal_redux.tbl_ihs_services',N'U') is not null  truncate table portal_redux.tbl_ihs_services;
insert into portal_redux.tbl_ihs_services
([id_ihs_episode]
      ,[dtl_id_payment_fact]
      ,[id_case]
      ,[id_prsn]
...
select @max_pay_id=max(id_payment_fact) from portal_redux.payment_fact
...
select eps.[id_ihs_episode]
	...
	join (select id_ihs_episode from portal_redux.TBL_ihs_episodes 
		except 
		select id_ihs_episode from portal_redux.TBL_ihs_services) q on q.id_ihs_episode=eps.id_ihs_episode



-- update 
update portal_redux.tbl_ihs_episodes
set cd_asgn_type= intk.cd_asgn_type
, tx_asgn_type=intk.tx_asgn_type
from #ihs_intk intk
where intk.ID_INTAKE_FACT=portal_redux.tbl_ihs_episodes.id_intake_fact
and tbl_origin_cd =3;
...
from  (
	...
	from  portal_redux.tbl_ihs_episodes eps 
	join portal_redux.tbl_child_episodes tce on tce.id_intake_fact=eps.id_intake_fact and tce.id_case=eps.id_case
...
from  (
	select distinct eps.id_ihs_episode, eps.id_case,eps.ihs_begin_date,eps.ihs_end_date,tce.state_custody_start_date ,federal_discharge_date
		,row_number() over (partition by eps.id_case,eps.ihs_begin_date order by tce.state_custody_start_date asc) as row_num
	from  portal_redux.tbl_ihs_episodes eps 
	join portal_redux.tbl_child_episodes tce on  tce.id_case=eps.id_case
		and tce.state_custody_start_date >= eps.ihs_begin_date
	where eps.ihs_end_date <> '12/31/3999' ) q 
where  q.id_ihs_episode=tbl_ihs_episodes.id_ihs_episode
	...
	from portal_redux.tbl_ihs_episodes eps
	join portal_redux.tbl_ihs_services svc on eps.id_ihs_episode=svc.id_ihs_episode
...
update statistics portal_redux.tbl_ihs_episodes

update statistics portal_redux.tbl_ihs_services
		--select ihs.id_ihs_episode,ihs.total_amt_paid,ihs.nbr_svc_paid
		--	,isnull(sum(svc.[am_total_paid]),0.00) as dtl_paid
		--	from portal_redux.tbl_ihs_episodes ihs
		--	join portal_redux.tbl_ihs_services svc on svc.id_ihs_episode=ihs.id_ihs_episode
