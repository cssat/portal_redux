...
-- exec portal_redux.[prod_build_ooh_dcfs_eps] '2014-08-08'			
...
--					, cast(nondcfs.CUST_End as datetime) as nonDCFS_Cust_End
				...
				from portal_redux.rptPlacement rpt 
					...
					join (select id_removal_episode_fact, derived_county, begin_date, end_date as last_placement_end_date
			...
			--   non-DCFS custody concatenate adjacent segments and exclude episodes contained within
				...
				where --**  select entry cohort
					...
					and ((removal_dt <='2000-01-01' and coalesce(rpt.discharge_dt,'12/31/9999') >='2000-01-01')
						or removal_dt >='2000-01-01')
...
CREATE NONCLUSTERED INDEX idx_tmp_33 on #dcfs_alleps (id_removal_episode_fact,[removal_dt],[federal_discharge_date])
		...
		join (			select distinct id_removal_episode_fact
						...
						from portal_redux.aoc_petition aoc
