-- create prm_eth_census view

CREATE  VIEW portal_redux.prm_eth_census AS 
...
from (portal_redux.ref_lookup_ethnicity_census ref_lookup_ethnicity
 join portal_redux.ref_xwalk_race_origin xwlk on((xwlk.cd_race_census = ref_lookup_ethnicity.cd_race_census))) 
 ...
 union select distinct xwlk.cd_race_census [cd_race]
		...
		,ref_lookup_ethnicity.cd_race_census [match_code]
			from (portal_redux.ref_xwalk_race_origin ref_lookup_ethnicity 
					 join portal_redux.ref_lookup_hispanic_latino_census  xwlk on((xwlk.census_hispanic_latino_origin_cd = ref_lookup_ethnicity.census_hispanic_latino_origin_cd))) 
					where (ref_lookup_ethnicity.cd_race_census between 1 and 8) 
union select distinct 0 AS cd_race,xwlk.census_hispanic_latino_origin_cd [cd_origin],xwlk.cd_race_census [match_code]
from  portal_redux.ref_xwalk_race_origin xwlk 
