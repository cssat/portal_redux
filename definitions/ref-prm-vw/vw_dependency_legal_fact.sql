-- create vw_dependency_legal_fact view

CREATE view [portal_redux].[vw_dependency_legal_fact]
as
with cte_dep as (
...
,lsd.CD_LGL_STAT
,lsd.TX_LGL_STAT
,portal_redux.INTDate_to_caldate(lf.ID_CALENDAR_DIM_EFFECTIVE) as eff_dt
,portal_redux.intDate_to_CalDate(lf.id_calendar_dim_hearing)  hearing_dt
, row_number() over (partition by lf.ID_CASE,lf.ID_PRSN
				,coalesce(portal_redux.intDate_to_CalDate(lf.ID_CALENDAR_DIM_EFFECTIVE)
									,portal_redux.intDate_to_CalDate(lf.id_calendar_dim_hearing))
 order by case when cd_result in (17,18,19,130) then 1 else 2 end  asc) as row_num
 from portal_redux.LEGAL_FACT LF
join portal_redux.LEGAL_ACTION_DIM lad on lf.ID_LEGAL_ACTION_DIM=lad.ID_LEGAL_ACTION_DIM
join portal_redux.LEGAL_RESULT_DIM lrd on lrd.ID_LEGAL_RESULT_DIM=lf.ID_LEGAL_RESULT_DIM
join portal_redux.LEGAL_STATUS_DIM lsd on lsd.ID_LEGAL_STATUS_DIM=lf.ID_LEGAL_STATUS_DIM
