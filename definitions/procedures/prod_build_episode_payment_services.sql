-- prod_build_episode_payment_services definition

-- EXEC portal_redux.prod_build_episode_payment_services;
-- DROP PROCEDURE IF EXISTS portal_redux.prod_build_episode_payment_services;
CREATE PROCEDURE portal_redux.prod_build_episode_payment_services
AS

BEGIN
		if object_id(N'portal_redux.episode_payment_services') is not null truncate table portal_redux.episode_payment_services
		insert into portal_redux.episode_payment_services    ([id_prsn_child]
		...
		SELECT [id_prsn_child]
		  ...
		  group by [id_prsn_child]
			 ...
			 --select * from portal_redux.tbl_child_episode_merge_id mrg where  mrg.merge_episode_id=126738

			 --select entry_date,id_case from portal_redux.tbl_child_placement_settings where id_removal_episode_fact=126738
