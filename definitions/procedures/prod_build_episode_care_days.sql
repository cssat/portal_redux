-- prod_build_episode_care_days definition

-- EXEC portal_redux.prod_build_episode_care_days;
-- DROP PROCEDURE IF EXISTS portal_redux.prod_build_episode_care_days;
CREATE PROCEDURE portal_redux.prod_build_episode_care_days
AS
...
BEGIN
	...
	DROP TABLE IF EXISTS #episodes;
			...
			select  distinct
						...
						,(select max(calendar_date) from portal_redux.CALENDAR_DIM where month(calendar_date)=month(rp.removal_dt)
...
--age_yrs_exit
						,isnull(iif(rp.discharge_dt <= cutoff_date,iif( rp.[18bday] <= rp.discharge_dt
										,portal_redux.fnc_datediff_yrs(birthdate,rp.[18bday])
										,portal_redux.fnc_datediff_yrs(birthdate,rp.discharge_dt)),-99),-99) age_yrs_exit
-- race
...
-- federal discharge date force
			...
			join portal_redux.rptPlacement rp 
			...
			left join portal_redux.PEOPLE_DIM pd on pd.ID_PRSN=rp.child and pd.IS_CURRENT=1
...
set care_day_cnt_prior_anniv=datediff(dd,iif(fy_start_date>=removal_dt,fy_start_date,removal_Dt),iif(removal_dt<>anniv_removal_dt,anniv_removal_dt,removal_dt))
		,prior_year_service=portal_redux.fnc_datediff_yrs(removal_dt,iif(removal_dt<>anniv_removal_dt,dateadd(dd,-1,anniv_removal_dt),removal_dt))
...
group by 
	...
	truncate table portal_redux.[episode_care_days]
	 insert into portal_redux.[episode_care_days](fiscal_yr,age_yrs_removal,age_yrs_exit,county_cd
...
--			join portal_redux.rptPlacement_Events rp 
--					on removal_dt <= cd.fy_stop_date
--			and  iif(portal_redux.lessorDate( [18bday],rp.discharge_dt) > cutoff_date
--				, cutoff_date
--				,portal_redux.lessorDate( [18bday],rp.discharge_dt))  >=cd.fy_start_date
--			and rp.plc_begin<= cd.fy_stop_date
--			and coalesce(rp.end_date,'12/31/3999') >=cd.fy_start_date
--			--where portal_redux.fnc_datediff_yrs(birthdate,removal_dt) = 0
--		--	and rp.derived_county=17
--group by fy_start_date

 --select fiscal_yr,sum(care_days) care_days
 --from portal_redux.episode_care_days
