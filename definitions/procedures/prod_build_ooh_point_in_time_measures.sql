-- prod_build_ooh_point_in_time_measures definition

-- EXEC portal_redux.prod_build_ooh_point_in_time_measures;
-- DROP PROCEDURE IF EXISTS portal_redux.prod_build_ooh_point_in_time_measures;
CREATE PROCEDURE portal_redux.prod_build_ooh_point_in_time_measures
AS
...
begin
	...
	set nocount on
						...
						SELECT evt.qry_type
							...
							into #evt
						  from  portal_redux.ooh_point_in_time_child evt
						  join portal_redux.ooh_dcfs_eps   eps 
				...
				from #evt evt
				join portal_redux.placement_payment_services pps on pps.id_placement_fact=evt.id_placement_fact
		...
		INCLUDE ([qualEvent])
			...
			if object_ID('tempDB..#siblings') is not null drop table #siblings;
				SELECT e.ID_PRSN_CHILD as ID_PRSN_PRIMCHILD
					...
					into  #siblings
					FROM portal_redux.SIBLING_RELATIONSHIP_FACT
			...
			ALTER TABLE  portal_redux.ooh_point_in_time_measures  NOCHECK CONSTRAINT ALL 
			 truncate table portal_redux.ooh_point_in_time_measures;
			
			insert into portal_redux.ooh_point_in_time_measures
			select [start_date]
			,date_type
			...
			ALTER TABLE  portal_redux.ooh_point_in_time_measures  CHECK CONSTRAINT ALL 

		update statistics portal_redux.ooh_point_in_time_measures;

			update portal_redux.prtl_tables_last_update
			set    [last_build_date] = getdate()
			  ,[row_count]=(select count(*) from portal_redux.ooh_point_in_time_measures)
		  FROM [portal_redux].[prtl_tables_last_update] where tbl_name='ooh_point_in_time_measures'
