-- prod_build_match_allegation definition

-- EXEC portal_redux.prod_build_match_allegation;
-- DROP PROCEDURE IF EXISTS portal_redux.prod_build_match_allegation;
CREATE PROCEDURE portal_redux.prod_build_match_allegation
AS
...
+  iif(NOT(fl_phys_abuse.fl_phys_abuse =0 and  fl_sexual_abuse.fl_sexual_abuse=0 and fl_neglect.fl_neglect=0)
		  ,fl_any_legal.fl_any_legal,0)  * (select cd_multiplier from portal_redux.ref_filter_allegation where fl_name='fl_any_legal') 
+ fl_phys_abuse.fl_phys_abuse * (select cd_multiplier from portal_redux.ref_filter_allegation where fl_name='fl_phys_abuse')
+  fl_sexual_abuse.fl_sexual_abuse * (select cd_multiplier from portal_redux.ref_filter_allegation where fl_name='fl_sexual_abuse')
+  fl_neglect.fl_neglect * (select cd_multiplier from portal_redux.ref_filter_allegation where fl_name='fl_neglect') 
,iif(NOT(fl_phys_abuse.fl_phys_abuse =0 and  fl_sexual_abuse.fl_sexual_abuse=0 and fl_neglect.fl_neglect=0)
		  ,fl_any_legal.fl_any_legal,0) fl_any_legal
,fl_phys_abuse.fl_phys_abuse
,fl_sexual_abuse.fl_sexual_abuse
,fl_neglect.fl_neglect
from (select cd_multiplier constant from portal_redux.ref_filter_allegation where cd_allegation=0) c
...
union select constant +  (select cd_multiplier from portal_redux.ref_filter_allegation where fl_name='fl_any_legal') ,1,0,0,0
from (select cd_multiplier constant from portal_redux.ref_filter_allegation where cd_allegation=0) c
union  select (select cd_multiplier constant from portal_redux.ref_filter_allegation where cd_allegation=0)  ,0,0,0,0
...
insert into portal_redux.ref_match_allegation ( cd_allegation , filter_allegation,fl_any_legal,fl_neglect,fl_sexual_abuse,fl_phys_abuse )
...
set last_build_date=getdate(),row_count=(select count(*) from portal_redux.ref_match_allegation)
where tbl_id=48;
