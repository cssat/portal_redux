-- prod_build_prtl_pbcs3 definition

-- EXEC portal_redux.prod_build_prtl_pbcs3;
-- DROP PROCEDURE IF EXISTS portal_redux.prod_build_prtl_pbcs3;
CREATE PROCEDURE portal_redux.prod_build_prtl_pbcs3
AS
...
set nocount on
	...
	join portal_redux.calendar_dim cd on cd.calendar_date=IHS.ihs_end_date 
	join portal_redux.vw_intakes_screened_in si on si.id_intake_fact=ihs.id_intake_fact
...
update #PBCS3_ALL set min_placed_within_month=case when min_placed_within_month is null then 48 else min_placed_within_month end    where placed=1 and daysIHSplc < 1461;
			...
			ALTER TABLE portal_redux.prtl_pbcs3 NOCHECK CONSTRAINT ALL;
			truncate table  portal_redux.prtl_pbcs3;

			insert into  portal_redux.prtl_pbcs3
				select 
					  cohort_begin_date 
					, date_type 
					, qry_type
			...
			ALTER TABLE portal_redux.prtl_pbcs3 CHECK CONSTRAINT ALL;
			update statistics portal_redux.prtl_pbcs3;

			update portal_redux.prtl_tables_last_update		
			set last_build_date=getdate()
				,row_count=(select count(*) from portal_redux.prtl_pbcs3)
			where tbl_id=7;	

			--select * from portal_redux.prtl_tables_last_update where tbl_id=7
			--select count(*) from portal_redux.prtl_pbcs3
