...
-- DROP PROCEDURE IF EXISTS portal_redux.prod_build_rate_placement_order_specific;
CREATE PROCEDURE portal_redux.prod_build_rate_placement_order_specific
AS
...
from #scrn_in intk
join portal_redux.ref_last_dw_transfer d on d.cutoff_date=d.cutoff_date
join (
			select eps.id_case
				,eps.id_intake_fact
				,eps.id_removal_episode_fact
				,(select top 1 rfrd_date from portal_redux.tbl_intakes intk where intk.id_intake_fact=eps.id_intake_fact) rfrd_date
				,eps.removal_dt
				,IIF(eps.discharge_dt> eps.[18bday]
								and eps.[18bday] < cutoff_date
						,eps.[18bday]
						,eps.discharge_dt) discharge_dt
			 from  portal_redux.rptPlacement   eps
			 ...
			 where eps.id_intake_fact is not null 
				...
				and not exists(select * from portal_redux.vw_nondcfs_combine_adjacent_segments nd 
	...
	where not exists(--exclude families with a child already in care
								select id_case from portal_redux.rptPlacement sic where sic.id_case=q.id_case
...
alter table portal_redux.rate_placement_order_specific NOCHECK constraint ALL;
truncate table portal_redux.rate_placement_order_specific;
insert into portal_redux.rate_placement_order_specific(date_type,qry_type,cohort_date,county_cd,nth_order,cnt_nth_order_placement_cases,cnt_prior_order_si_referrals,placement_rate)
...
from    (select distinct [month] from portal_redux.calendar_dim 
				where calendar_date between '2004-01-01' and (select dateadd(m,-1,(select cutoff_date from portal_redux.ref_last_dw_transfer)))
				) mnth
 join portal_redux.ref_lookup_county refC on refC.county_cd between 0 and 39
 join (select number nth_order from portal_redux.numbers ) n on n.nth_order between 1 and 3
...
left join  (-- this is the denominator 
		...
		,n.nth_order
		from #scrn_in si
		join portal_redux.prm_cnty cnty on cnty.match_code=si.intake_county_cd
		 join (select number nth_order from portal_redux.numbers ) n on n.nth_order between 1 and 3
