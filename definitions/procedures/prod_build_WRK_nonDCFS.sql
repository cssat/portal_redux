-- prod_build_WRK_NonDCFS definition

-- EXEC portal_redux.prod_build_WRK_NonDCFS;
-- DROP PROCEDURE IF EXISTS portal_redux.prod_build_WRK_NonDCFS;
CREATE PROCEDURE portal_redux.prod_build_WRK_NonDCFS
AS
...
BEGIN
	...
	SELECT DISTINCT ID_PLACEMENT_CARE_AUTH_FACT
		...
		,portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_BEGIN) AS cust_begin
		,portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_END) AS cust_end
		,CASE 
			WHEN ID_CALENDAR_DIM_END = 0
				THEN 999999
			ELSE datediff(dd, portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_BEGIN), portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_END))
			END AS custlos
		,CASE 
		...
		,row_number() OVER (
			...
			,portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_BEGIN) ORDER BY ID_PLACEMENT_CARE_AUTH_FACT
				,CASE 
					WHEN ID_CALENDAR_DIM_END = 0
						THEN 999999
					ELSE datediff(dd, portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_BEGIN), portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_END))
	...
	INTO #AUTH
	FROM portal_redux.PLACEMENT_CARE_AUTH_FACT pcaf
	JOIN portal_redux.PLACEMENT_CARE_AUTH_DIM pcad ON pcad.ID_PLACEMENT_CARE_AUTH_DIM = pcaf.ID_PLACEMENT_CARE_AUTH_DIM
	WHERE CD_PLACEMENT_CARE_AUTH <> - 99
		AND ID_PRSN >= 0
		AND portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_BEGIN) IS NOT NULL
		AND ID_CALENDAR_DIM_BEGIN >= 19980101
		AND FL_VOID = 0
	ORDER BY ID_PRSN
		,portal_redux.IntDate_to_CalDate(ID_CALENDAR_DIM_BEGIN) ASC
	...
	IF object_id(N'portal_redux.WRK_nonDCFS_All', N'U') IS NOT NULL
	...
	INTO portal_redux.WRK_nonDCFS_All
	FROM #AUTH
	WHERE custc = 0
	ORDER BY ID_PRSN
		,CUST_BEGIN;

	UPDATE portal_redux.WRK_nonDCFS_All
	...
	INTO #nondcfs
	FROM portal_redux.WRK_nonDCFS_All
	ORDER BY id_prsn
		,CD_PLACEMENT_CARE_AUTH
