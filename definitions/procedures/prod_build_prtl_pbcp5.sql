-- prod_build_prtl_pbcp5 definition

-- EXEC portal_redux.prod_build_prtl_pbcp5;
-- DROP PROCEDURE IF EXISTS portal_redux.prod_build_prtl_pbcp5;
CREATE PROCEDURE portal_redux.prod_build_prtl_pbcp5
AS
...
begin
	...
	into #eps	
	from portal_redux.ooh_dcfs_eps tce
...
join portal_redux.SERVICE_TYPE_DIM std on std.ID_SERVICE_TYPE_DIM=pf.ID_SERVICE_TYPE_DIM
join  #eps eps on portal_redux.IntDate_to_CalDate(pf.ID_CALENDAR_DIM_SERVICE_BEGIN) between eps.Federal_Discharge_Date  and dateadd(dd,90,Federal_Discharge_Date)
			and eps.cd_discharge_type=3
...
join portal_redux.rptPlacement rpt on rpt.child=adp.ID_PRSN_CHILD
...
-- insert first removals
	...
	group by 
		cohort_exit_year
			...
			ALTER TABLE portal_redux.prtl_pbcp5 NOCHECK CONSTRAINT ALL 
			truncate table portal_redux.prtl_pbcp5


			insert into portal_redux.prtl_pbcp5 (
				cohort_exit_year
				, date_type
				,  qry_type
				,  cd_discharge_type
			...
			ALTER TABLE portal_redux.prtl_pbcp5 CHECK CONSTRAINT ALL ;

			update statistics portal_redux.prtl_pbcp5;
