-- prod_update_rptPlacement_with_intakes definition

-- EXEC portal_redux.prod_update_rptPlacement_with_intakes;
-- DROP PROCEDURE IF EXISTS portal_redux.prod_update_rptPlacement_with_intakes;
CREATE PROCEDURE portal_redux.prod_update_rptPlacement_with_intakes
AS
...
BEGIN
	 ...
	 -- intakes from TBL_INTAKES filtering out alternate intervention and reopen cases
		  -- where referral date within a year prior to state custody start date thru 10 days beyond state custody start date
			...
			into #episodes
			from portal_redux.rptPlacement tce
			left join portal_redux.TBL_INTAKES si on si.id_case=tce.id_case and si.cd_final_decision=1   and si.id_case > 0
	...
	--		select * from #episodes eps where exists(select * from #episodes mult where mult.row_num > 1  and mult.id_removal_episode_fact=eps.id_removal_episode_fact)
			...
			join (select  ivf.ID_CASE
							...
							from portal_redux.INTAKE_VICTIM_FACT ivf
							join portal_redux.tbl_intakes	 intk  on ivf.ID_INTAKE_FACT=intk.ID_INTAKE_FACT  and intk.id_case > 0
			...
			join (
					...
					join (	select distinct eps1.child,eps1.id_case
								from portal_redux.rptPlacement eps1
								where exists(select * from portal_redux.rptPlacement eps2 where eps2.child=eps1.child and eps2.id_case <> eps1.id_case)
								--order by child
								) q2 on q2.child=tce.child and q2.ID_CASE <> tce.ID_CASE_TCE
					join portal_redux.TBL_INTAKES si on si.ID_CASE = q2.ID_CASE and si.cd_final_decision=1  and fl_alternate_intervention=0 and fl_reopen_case=0 and fl_dlr=0
							 and si.id_case > 0
		...
		-- where the referral date within a year prior to state custody start date thru 10 days beyond state custody start date
			...
			join (
					select distinct tce.id_removal_episode_fact
						...
						join portal_redux.TBL_INTAKES si on si.id_intake_fact=q.id_intake_fact and si.cd_final_decision=1 and fl_alternate_intervention=0 and fl_reopen_case=0 and fl_dlr=0
			...
			join (
						...
						from #episodes tce
						 join portal_redux.allegation_fact alf on alf.id_prsn_victim=tce.child
						 join portal_redux.abuse_type_dim atd on atd.id_abuse_type_dim=alf.id_abuse_type_dim
						 join portal_redux.tbl_intakes si on si.id_intake_fact=alf.id_intake_fact  and si.cd_final_decision=1 and fl_reopen_case=0 and fl_dlr=0
						  and si.id_case > 0
						 where tce.id_intake_fact is null 
						 and  si.inv_ass_start between dateadd(dd,-365,removal_dt) and dateadd(dd,10,removal_dt)
			 ) qry on qry.id_removal_episode_fact=eps.id_removal_episode_fact and qry.row_num=1
			...
			join (
						...
						,row_number() over (partition by ID_REMOVAL_EPISODE_FACT
											order by ID_REMOVAL_EPISODE_FACT,isnull(si.inv_ass_start,removal_dt) desc) as row_num
						from #episodes tce
						 join portal_redux.TBL_INTAKES si on si.id_case=tce.ID_CASE_TCE  and si.cd_final_decision=1 and fl_reopen_case=0 and fl_alternate_intervention=1 and fl_dlr=0
						  and si.id_case > 0
						 where tce.id_intake_fact is null 
						 and  si.inv_ass_start between dateadd(dd,-365,removal_dt) and dateadd(dd,10,removal_dt)
			 ) qry on qry.ID_REMOVAL_EPISODE_FACT=eps.ID_REMOVAL_EPISODE_FACT and qry.row_num=1
			...
			join (
					select distinct tce.ID_REMOVAL_EPISODE_FACT,tce.ID_CASE_TCE ,child,si.id_intake_fact,si.ID_CASE,si.inv_ass_start,si.inv_ass_stop,removal_dt,discharge_dt
						,row_number() over (partition by ID_REMOVAL_EPISODE_FACT
											order by ID_REMOVAL_EPISODE_FACT,isnull(si.inv_ass_start,removal_dt) desc) as row_num
					from #episodes tce
					join portal_redux.TBL_INTAKES si on si.ID_CASE=tce.ID_CASE_TCE  and si.cd_final_decision=1  and fl_reopen_case=1 and si.fl_dlr=0
					 and si.id_case > 0
					where tce.ID_INTAKE_FACT is null 
						and si.inv_ass_start between dateadd(dd,-365,removal_dt) and dateadd(dd,10,removal_dt)
					) qry on qry.ID_REMOVAL_EPISODE_FACT=eps.ID_REMOVAL_EPISODE_FACT and qry.row_num=1
			...
			join (
					...
					join (		select distinct eps1.child,eps1.id_case
								from portal_redux.rptPlacement eps1
								where exists(select * from portal_redux.rptPlacement plc where plc.child=eps1.child and plc.id_case <> eps1.id_case)
								) q2 on q2.child=tce.child and q2.ID_CASE <> tce.ID_CASE_TCE
					join portal_redux.TBL_INTAKES si on si.ID_CASE = q2.ID_CASE   and si.cd_final_decision=1 and fl_reopen_case=1 and si.fl_dlr=0
					 and si.id_case > 0
					where tce.ID_INTAKE_FACT is null 
						and si.inv_ass_start between dateadd(dd,-365,tce.removal_dt) and dateadd(dd,10,tce.removal_dt)
					) qry on qry.ID_REMOVAL_EPISODE_FACT=eps.ID_REMOVAL_EPISODE_FACT and qry.row_num=1
			...
			SELECT distinct tcps.id_prsn_child as ID_PRSN_PRIMCHILD
						...
						, ID_PRSN_SIBLING
						, ID_CASE_CHILD  
						, portal_redux.IntDate_To_CalDate(id_calendar_dim_begin) as sibrel_begin
						, portal_redux.IntDate_To_CalDate(id_calendar_dim_end) as sibrel_end
				...
				into  #siblings
				FROM portal_redux.SIBLING_RELATIONSHIP_FACT sib
				join portal_redux.TBL_CHILD_PLACEMENT_SETTINGS tcps on tcps.id_placement_fact=sib.ID_PLACEMENT_FACT
				join #episodes eps on eps.ID_REMOVAL_EPISODE_FACT=tcps.id_removal_episode_fact
				--where portal_redux.IntDate_To_CalDate(id_calendar_dim_begin) <= coalesce(e.discharge_dt,'12/31/3999')
				--	and isnull(portal_redux.IntDate_To_CalDate(id_calendar_dim_end),'12/31/3999') > e.removal_dt
