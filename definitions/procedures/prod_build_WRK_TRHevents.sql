-- prod_build_WRK_TRHEvents definition

-- EXEC portal_redux.prod_build_WRK_TRHEvents;
-- DROP PROCEDURE IF EXISTS portal_redux.prod_build_WRK_TRHEvents;
CREATE PROCEDURE portal_redux.prod_build_WRK_TRHEvents
AS
...
BEGIN
		...
		left join (select tbl_child_placement_settings.id_removal_episode_fact as iref_pl
							...
							from portal_redux.TBL_CHILD_PLACEMENT_SETTINGS 
							join portal_redux.TBL_CHILD_EPISODES tce on tce.id_removal_episode_fact=tbl_child_placement_settings.id_removal_episode_fact
		...
		if object_ID(N'portal_redux.WRK_TRHEvents',N'U') is not null drop table portal_redux.WRK_TRHEvents;
		...
		into portal_redux.WRK_TRHEvents
		from  #WRK_TRHEvents
