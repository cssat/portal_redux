-- prod_build_rate_referrals_screened_in_order_specific definition

-- EXEC portal_redux.prod_build_rate_referrals_screened_in_order_specific;
-- DROP PROCEDURE IF EXISTS portal_redux.prod_build_rate_referrals_screened_in_order_specific;
CREATE PROCEDURE portal_redux.prod_build_rate_referrals_screened_in_order_specific
AS
	...
	--		
			...
			join (select distinct [month] from portal_redux.calendar_dim 
					where calendar_date between '2000-01-01' 
								and (select dateadd(m,-1,(select cutoff_date from portal_redux.ref_last_dw_transfer)))
			...
			join portal_redux.prm_cnty cnty on cnty.match_code=curr.intake_county_cd
		...
		--this is the count by  "Nth" order screened in referral...
			...
			into #referrals
			from #ref scrn
			join portal_redux.prm_cnty cnty on cnty.match_code=scrn.intake_county_cd
...
alter table portal_redux.rate_referrals_scrn_in_order_specific NOCHECK CONSTRAINT ALL
truncate table portal_redux.rate_referrals_scrn_in_order_specific
insert into portal_redux.rate_referrals_scrn_in_order_specific
