-- prod_build_prtl_poc3ab definition

-- EXEC portal_redux.prod_build_prtl_poc3ab;
-- DROP PROCEDURE IF EXISTS portal_redux.prod_build_prtl_poc3ab;
CREATE PROCEDURE portal_redux.prod_build_prtl_poc3ab
AS
...
begin	
	  ...
	  set nocount on 		
		...
		set @chend = (select distinct dateadd(dd,-1,[quarter]) from portal_redux.CALENDAR_DIM where @cutoff_date = CALENDAR_DATE)
			...
			insert into #dates
			select distinct [Year],dateadd(dd,-1,dateadd(yy,1,[Year])),2 from portal_redux.calendar_dim
			where [year] between @chstart and (select end_date from [portal_redux].[ref_last_month_qtr_yr] where date_type=2)
		...
		--unique temp table
			...
			into #poc3_all
			from portal_redux.tbl_ihs_episodes ce
						join portal_redux.vw_intakes_screened_in si on si.id_intake_fact=ce.id_intake_fact
						 join portal_redux.ref_xwlk_reporter_type rpt on rpt.cd_reporter_type=si.cd_reporter	
			where fl_first_IHS_after_intake = 1
...
INCLUDE ([cnt_distinct_Intakes])
	...
	truncate table portal_redux.prtl_poc3ab;
	INSERT INTO portal_redux.[prtl_poc3ab]
           ([qry_type]
           ,[date_type]
           ,[start_date]
           ,[start_year]
	...
	update portal_redux.prtl_poc3ab
	...
	where q.qry_type=prtl_poc3ab.qry_type
		...
		update statistics portal_redux.prtl_poc3ab;
	  update portal_redux.prtl_tables_last_update		
	  set last_build_date=getdate()
	  ,row_count=(select count(*) from portal_redux.prtl_poc3ab)
	  where tbl_id=3;	
