-- prod_build_match_srvc_type_budget definition

-- EXEC portal_redux.prod_build_match_srvc_type_budget;
-- DROP PROCEDURE IF EXISTS portal_redux.prod_build_match_srvc_type_budget;
CREATE PROCEDURE portal_redux.prod_build_match_srvc_type_budget
AS

BEGIN

if object_ID(N'portal_redux.ref_match_srvc_type_budget',N'U') is not null drop table portal_redux.ref_match_srvc_type_budget

create table portal_redux.ref_match_srvc_type_budget(filter_service_budget int,[fl_budget_C12] int,[fl_budget_C14] int,[fl_budget_C15] int
...
insert into portal_redux.ref_match_srvc_type_budget(filter_service_budget ,[fl_budget_C12] ,[fl_budget_C14] ,[fl_budget_C15] 
...
SELECT distinct       power(10.0,7) +
  ...
  FROM [portal_redux].[episode_payment_services]
  union 
  select distinct
   power(10.0,7) +
				   ( (fl_budget_C12 * 1000000) + 
      ...
      ,[fl_budget_C19]
      ,[fl_uncat_svc],0
	  from portal_redux.tbl_ihs_episodes
	  union
	  select distinct 
	  ...
	  select ref.filter_service_budget
			...
			,q.cd_budget_poc_frc
	  into #temp
	  from portal_redux.ref_match_srvc_type_budget ref
	...
	truncate table portal_redux.ref_match_srvc_type_budget
...
,0 ,0 ,0 ,0,0;
	...
	join (select s.filter_service_budget,s.cd_budget_poc_frc
			,row_number() over (order by filter_service_budget ) as id_srvc
				  from portal_redux.ref_match_srvc_type_budget s) s on s.filter_service_budget=ref.filter_service_budget
				 


update portal_redux.prtl_tables_last_update
set last_build_date=getdate(),row_count=(select count(*) from portal_redux.ref_match_srvc_type_budget)
where tbl_id=50;
