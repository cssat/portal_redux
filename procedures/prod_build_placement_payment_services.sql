-- prod_build_placement_payment_services definition

-- EXEC portal_redux.prod_build_placement_payment_services;
-- DROP PROCEDURE IF EXISTS portal_redux.prod_build_placement_payment_services;
CREATE PROCEDURE portal_redux.prod_build_placement_payment_services
AS
...
DROP TABLE IF EXISTS #pay;
  select id_payment_fact
			...
			and  (portal_redux.IntDate_to_CalDate( pf.ID_CALENDAR_DIM_SERVICE_BEGIN) >='2000-01-01'
				or isnull(portal_redux.IntDate_to_CalDate( pf.ID_CALENDAR_DIM_SERVICE_END) ,'12/31/3999') >'2000-01-01' )
...
create index idx_pay on #pay(id_prsn_child,[srvc_begin_dt],[srvc_end_dt]) 
	...
	/*******  STEP 1   match person provider cd_srvc dates  ******************************************************/
		insert into portal_redux.placement_payment_services
		   ([id_prsn_child]
           ,[begin_date]
           ,[end_date]
		...
		select  
					...
					, tcps.id_case
					, iif(tcps.days_to_dsch>0,tcps.days_to_dsch + 1,iif(tcps.end_date <> '9999-12-31' and  tcps.end_date is not null,datediff(dd,tcps.begin_date,tcps.end_date) + 1,datediff(dd,tcps.begin_date,(select cutoff_date from portal_redux.ref_last_dw_transfer)))) [dur_days]
					...
					, pps.fl_plc_svc
					, 0 [fl_dup_payment]
			from portal_redux.rptPlacement_Events tcps 
			...
			from portal_redux.placement_payment_services PP
					...
					,pps.pymt_tx_srvc,pps.svc_begin_date,pps.svc_end_date
					 ...
					 join (
							select id_payment_fact,count(*) as cnt
							  from  portal_redux.placement_payment_services
			...
			delete PP from portal_redux.placement_payment_services PP where fl_dup_payment=1
	...
	/*******  STEP 2   		--now match on child dates provider but NOT service  ********************************/
		...
		select  distinct
			...
			join portal_redux.ref_service_category pps on pps.cd_srvc=std.cd_srvc  and pps.fl_plc_svc=1
			 where  not exists( select *
							from portal_redux.tbl_ihs_services ihs 
							where  ihs.dtl_id_payment_fact=pf.ID_PAYMENT_FACT )
				and not exists(select * 
				from portal_redux.placement_payment_services plc 
...
/***************  
update to table portal_redux.placement_payment_services
...
from portal_redux.placement_payment_services pps
join (
select id_payment_fact,count(*)   as cnt  from portal_redux.placement_payment_services
