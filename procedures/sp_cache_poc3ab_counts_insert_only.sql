...
CREATE PROCEDURE portal_redux.[sp_cache_poc3ab_counts_insert_only](
...
BEGIN
 set nocount on
	...
	update statistics #ihs
		...
		set @qry_id=(
		select top 1 qry_id from portal_redux.cache_poc3ab_params
		...
		begin
			...
			INSERT INTO portal_redux.[cache_poc3ab_params]
					(qry_id
					, cd_sib_age_grp
					,[cd_race_census]
					,cd_county
				...
				select 
					isnull((select max(qry_id) +1
						from portal_redux.[cache_poc3ab_params]),1)
					,@age_grouping_cd
					,@race_cd
					,@cd_county
					,@bin_ihs_svc_cd
			...
			begin
					...
					update portal_redux.cache_poc3ab_params
		   ...
		   select (cast((int_param_key  * power(10.0,6)) as decimal(22,0))
			...
			join portal_redux.[cache_qry_param_poc3ab] poc3ab
		...
		if @var_row_cnt_param <> @var_row_cnt_cache
			begin
				insert into portal_redux.cache_poc3ab_aggr ( 
					[qry_type]
					,[date_type]
					,[start_date]
					,[int_param_key]
				...
				SELECT    prtl_poc3ab.qry_type
					...
					group by  prtl_poc3ab.qry_type
						...
						from portal_redux.cache_poc3ab_aggr,prm_household_census_population   
				...
				insert into portal_redux.cache_qry_param_poc3ab
