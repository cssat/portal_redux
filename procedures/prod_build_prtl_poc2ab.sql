-- prod_build_prtl_poc2ab definition

-- EXEC portal_redux.prod_build_prtl_poc2ab;
-- DROP PROCEDURE IF EXISTS portal_redux.prod_build_prtl_poc2ab;
CREATE PROCEDURE portal_redux.prod_build_prtl_poc2ab
AS
...
BEGIN
	...
	select		distinct
		...
		into #intakes
		from portal_redux.vw_intakes_screened_in  tce 
		join #mydates on 1=1
			...
			select 
				[qry_type]
				   ...
				   , (select cd_multiplier from portal_redux.ref_filter_access_type where cd_access_type=0 )
				   							+  fl_dlr * (select cd_multiplier from portal_redux.ref_filter_access_type where fl_name='fl_dlr')
