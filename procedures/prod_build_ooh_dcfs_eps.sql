-- prod_build_ooh_dcfs_eps definition

-- EXEC portal_redux.prod_build_ooh_dcfs_eps;
-- DROP PROCEDURE IF EXISTS portal_redux.prod_build_ooh_dcfs_eps;
CREATE PROCEDURE portal_redux.prod_build_ooh_dcfs_eps
AS
...
--					, cast(nondcfs.CUST_End as datetime) as nonDCFS_Cust_End
				...
				from portal_redux.rptPlacement rpt
					join portal_redux.PEOPLE_DIM pd on pd.id_prsn=rpt.child and IS_CURRENT=1
					join portal_redux.calendar_dim cd on cd.id_calendar_dim=rpt.id_calendar_dim_begin 
					...
					join (select id_placement_fact, id_removal_episode_fact, begin_date, derived_county,end_date as last_placement_end_date
							,row_number() over (partition by id_removal_episode_fact
												...
												from  portal_redux.rptPlacement_Events  ) evt on evt.id_removal_episode_fact=rpt.id_removal_episode_fact
													and evt.row_num_desc=1
					left join portal_redux.age_dim AgeEntry on rpt.age_at_removal_mos =AgeEntry.age_mo  
					left join portal_redux.vw_intakes_screened_in si on si.id_intake_fact=rpt.id_intake_fact
					left join portal_redux.tbl_ihs_episodes ihs on ihs.id_intake_fact=rpt.id_intake_fact and ihs.id_case=rpt.id_case
					left join portal_redux.ref_xwlk_reporter_type rpt_intk on rpt_intk.cd_reporter_type=si.cd_reporter
					left join portal_redux.ref_lookup_gender G on G.pk_gndr=rpt.pk_gndr
					left join portal_redux.ref_lookup_cd_discharge_type_exits dsch on dsch.cd_discharge_type=rpt.cd_discharge_type
				-- non-DCFS custody start date discharge date within episode
					left join portal_redux.vw_nondcfs_combine_adjacent_segments nondcfs_eps_within on nondcfs_eps_within.ID_PRSN=rpt.child
			...
			-- non-DCFS custody overlaps
					left join portal_redux.vw_nondcfs_combine_adjacent_segments nondcfs_overlap on nondcfs_overlap.ID_PRSN=rpt.child
			...
			--   non-DCFS custody concatenate adjacent segments and exclude episodes contained within
					left join portal_redux.vw_nondcfs_combine_adjacent_segments
...
--		select  removal_dt,Federal_Discharge_Date ,dateadd(yy,18,birthdate)  	from #eps
		...
		from #eps eps
		 join portal_redux.vw_nondcfs_combine_adjacent_segments nondcfs on nondcfs.ID_PRSN=eps.child
			...
			and nondcfs.CUST_END between eps.removal_dt
				and coalesce(eps.federal_discharge_date,'12/31/9999')
		join portal_redux.CALENDAR_DIM cd on cd.CALENDAR_DATE=nondcfs.cust_end
		...
		from #eps eps
		join portal_redux.CALENDAR_DIM cd on cd.CALENDAR_DATE=removal_dt
		...
		from #eps eps
		join portal_redux.CALENDAR_DIM cd on cd.CALENDAR_DATE=Federal_Discharge_Date
...
--  select child,removal_dt,count(*) from #eps group by child,removal_dt having count(*) > 1
		...
		join portal_redux.vw_nondcfs_combine_adjacent_segments dcfs on dcfs.id_prsn=eps.child
	...
	-- begin split segments
		...
		begin
			...
			insert into #tmp(id_removal_episode_fact,removal_dt,federal_discharge_date,fl_multiple)
			select  n1.id_removal_episode_fact,n1.cust_end,portal_redux.lessorDate(n2.federal_discharge_date,n2.cust_begin)
			,1 as fl_multiple
			from #nondcfs n1
		...
		insert into #tmp(id_removal_episode_fact,removal_dt,federal_discharge_date,fl_multiple)
		select  n1.id_removal_episode_fact,n2.cust_end,portal_redux.lessorDate(n1.federal_discharge_date,n1.cust_begin),1 as fl_multiple
		from #nondcfs n1
		...
		from portal_redux.CALENDAR_DIM 
	...
	from #dcfs_alleps eps
	join #age age on age.id_removal_episode_fact=eps.id_removal_episode_fact   and age.federal_discharge_date=eps.federal_discharge_date and age.removal_dt=eps.removal_dt
	join portal_redux.age_dim AgeExit on AgeExit.age_mo =age.age_mo
	join  (select max(cdc_census_mix_age_cd) [cdc_census_mix_age_cd] from portal_redux.age_dim where age_yr=17) cma on 1=1
	join (select max(census_child_group_cd) [census_child_group_cd] from portal_redux.age_dim where age_yr=17) ccg on 1=1
...
--- LOH 9/18
	...
	join portal_redux.ref_age_developmental ref_age  on ref_age.age_mo=age.age_mo
	join (select max(developmental_age_cd) [developmental_age_cd] from portal_redux.age_dim where age_yr=17) da on 1=1
	where eps.federal_discharge_date is not null and eps.federal_discharge_date < '9999-12-31'
...
CREATE NONCLUSTERED INDEX idx_tmp_33 on #dcfs_alleps (id_removal_episode_fact,[removal_dt],[federal_discharge_date])
		...
		from #dcfs_alleps eps
		 join (
				...
				join portal_redux.ref_filter_los los on datediff(dd,removal_dt,case when federal_discharge_date = '12/31/9999' 
		...
		--select rpt.id_removal_episode_fact,child,rpt.removal_dt,rpt.federal_discharge_date,q.days_frm_state_custody,q.petition_date as petition_dependency_date
		from #dcfs_alleps rpt		
			join portal_redux.ref_filter_dependency dep on datediff(dd,rpt.removal_dt,dependency_dt) between diff_days_from and diff_days_thru and dep.bin_dep_cd > 1
		...
		--  if object_ID(N'portal_redux.ooh_eps',N'U') is not null truncate table  portal_redux.ooh_eps
		if object_ID(N'portal_redux.ooh_dcfs_eps',N'U') is not null truncate table  portal_redux.ooh_dcfs_eps
		

		insert into portal_redux.ooh_dcfs_eps
...
update portal_redux.prtl_tables_last_update
	  ...
	  ,row_count=(select count(*) from portal_redux.ooh_dcfs_eps)
where tbl_id=39
