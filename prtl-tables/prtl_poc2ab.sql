-- create prtl_poc2ab table

-- DROP TABLE portal_redux.prtl_poc2ab;
...
CREATE procedure  [portal_redux].[prod_build_prtl_poc2ab](@permission_key datetime) 
as 
set nocount on
...
begin
	...
	into #cohortDates
	from portal_redux.CALENDAR_DIM,#mydates
	where [quarter] between startDate and enddate and date_type=1
	union 
	select distinct [YEAR], date_type from portal_redux.CALENDAR_DIM, #mydates
	...
	join #mydates on 1=1
	join portal_redux.CALENDAR_DIM ent on ent.ID_CALENDAR_DIM= convert(int,convert(varchar(8),tce.inv_ass_start,112))
	left join portal_redux.calendar_dim ext on ext.ID_CALENDAR_DIM=convert(int,convert(varchar(8),tce.inv_ass_stop,112)) and tce.inv_ass_stop is not null
	left join portal_redux.CALENDAR_DIM ent_intk_grp on ent_intk_grp.ID_CALENDAR_DIM= convert(int,convert(varchar(8),tce.intk_grp_inv_ass_start,112))
	left join portal_redux.calendar_dim ext_intk_grp on ext_intk_grp.ID_CALENDAR_DIM=convert(int,convert(varchar(8),tce.intk_grp_inv_ass_stop,112))
		and tce.intk_grp_inv_ass_stop is not null
	-- left join portal_redux.ref_lookup_ethnicity_census RC on RC.cd_race_census=tce.cd_race_census
	left join portal_redux.ref_xwlk_reporter_type rpt on rpt.cd_reporter_type=tce.cd_reporter
	left join portal_redux.ref_xwlk_reporter_type rpt2 on rpt2.cd_reporter_type=tce.intk_grp_cd_reporter_type
	WHERE tce.id_case>0 
	...
	join (select *, row_number() over (
        ...
        id_intake_fact asc) row_num
        from #intakes) q on  q.id_intake_fact=intk.id_intake_fact and row_num=1 
...
/******************************** qry_type ALL *********************************************/
	...
	join #cohortDates  cd on intk_grp_inv_ass_start < cd.[cohort_date]
	...
	join (select
		...
		join #cohortDates cd on inv_ass_start < cd.[cohort_date]
			and inv_ass_stop >= cd.[cohort_date]
			and md.date_type=cd.date_type) q on q.row_num=1
        and q.id_intake_fact = #intakes.id_intake_fact 
...
CREATE NONCLUSTERED INDEX idx_insert_qry_poc2
	...
	alter table portal_redux.prtl_poc2ab NOCHECK CONSTRAINT ALL;
	if object_ID(N'portal_redux.prtl_poc2ab',N'U') is not null truncate table portal_redux.prtl_poc2ab;
			insert into portal_redux.prtl_poc2ab
			  ([qry_type]
           ,[date_type]
           ,[start_date]
           ,[start_year]
