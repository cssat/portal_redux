...
CREATE PROCEDURE [portal_redux].[sp_ooh_flow_exits](
   @date varchar(3000)
...
as
 ...
 declare @qry_id bigint;
	...
	select cd_discharge_type,match_code from portal_redux.prm_dsch_exits;	


    -----------------------------------  set dates  -------------------------------------  		
    select @minmonthstart=min_date_any ,@maxmonthstart=max_date_any ,@mindate=min_date_any,@maxdate=max_date_any
	  FROM portal_redux.ref_lookup_max_date where id=19;
...
from (
	...
	select cast('2000-01-01' as datetime) as 'db_min_filter_date'
		...
		set @qry_id=(
		select top 1 qry_id from portal_redux.cache_poc1ab_exits_params
		...
		begin
			...
			INSERT INTO [portal_redux].[cache_poc1ab_exits_params]
					(qry_id
					, [age_grouping_cd]
					,[cd_race_census]
					,[pk_gndr]
				...
				select 
					isnull((select max(qry_id) +1
						from portal_redux.[cache_poc1ab_exits_params]),1)
					,@age_grouping_cd
					,@race_cd
					,@gender_cd
					,@init_cd_plcm_setng
				...
				begin
						update portal_redux.cache_poc1ab_exits_params
		...
		if OBJECT_ID('tempDB..#cachekeys') is not null drop table #cachekeys;
		   ...
		   select	([int_param_key] * power(10.0,9) ) +
			...
			join [portal_redux].[cache_qry_param_poc1ab_exits] poc1ab_exits
	...
	if @var_row_cnt_param <> @var_row_cnt_cache
			begin
							...
							insert into portal_redux.cache_poc1ab_exits_aggr( 
								[qry_type]
								,[date_type]
								,[start_date]
								,[int_param_key]
						...
						SELECT    prtl_poc1ab_exits.qry_type
								...
								,1
							FROM portal_redux.prtl_poc1ab_exits  
						...
						from  portal_redux.cache_poc1ab_exits_aggr  
						, portal_redux.ref_lookup_census_population  
...
--							or  cache_poc1ab_exits_aggr.cnt_exits > pop_cnt * .35 
						...
						update statistics portal_redux.cache_poc1ab_exits_aggr
						insert into portal_redux.cache_qry_param_poc1ab_exits
									   ([int_param_key]
									   ,bin_dep_cd
								   ,[bin_los_cd]
								   ,[bin_placement_cd]
						...
						update statistics portal_redux.cache_qry_param_poc1ab_exits;
	end
	IF @fl_return_results=1
	 select            
        ...
        and ck.int_hash_key=poc1ab.int_hash_key
			join portal_redux.ref_age_census_child_group  ref_age on ref_age.age_grouping_cd=poc1ab.age_grouping_cd
            join portal_redux.ref_filter_dependency ref_dep on ref_dep.bin_dep_cd=poc1ab.bin_dep_cd
        ...
        join portal_redux.ref_lookup_cd_discharge_type_exits dsch on dsch.cd_discharge_type=poc1ab.cd_discharge_type
