-- exec statement
EXEC portal_redux.sp_ihs_safety
	@date = '0'
,	@age_grouping_cd = '0'
,	@race_cd = '0'
,	@cd_county = '0'
...
CREATE PROCEDURE [portal_redux].[sp_ihs_safety](
   @date varchar(3000)
...
set nocount on
    ...
    -----------------------------------  set dates  -------------------------------------  		
	   ...
	   FROM portal_redux.ref_lookup_max_date where procedure_name='sp_ihs_safety';
	...
	join portal_redux.fn_ReturnStrTableFromList(@cd_reporter_type,1) sel
	...
	join portal_redux.fn_ReturnStrTableFromList(@filter_allegation,1) sel
	...
	update statistics #ihs
		...
		set @qry_id=(
		select top 1 qry_id from portal_redux.cache_pbcs3_params
		...
		begin
			...
			INSERT INTO [portal_redux].[cache_pbcs3_params]
					(qry_id
					, cd_sib_age_grp
					,[cd_race_census]
					,cd_county
				...
				select 
					isnull((select max(qry_id) +1
						from portal_redux.[cache_pbcs3_params]),1)
					,@age_grouping_cd
					,@race_cd
					,@cd_county
					,@bin_ihs_svc_cd
			...
			begin
					...
					update portal_redux.cache_pbcs3_params
		   ...
		   select (cast((int_param_key  * power(10.0,6)) as decimal(22,0))
			...
			join portal_redux.[cache_qry_param_pbcs3] pbcs3
  ...
  into #total
  FROM portal_redux.[prtl_pbcs3] s3
...
into #mytemp
  FROM portal_redux.[prtl_pbcs3]
	...
	join #cachekeys che on che.int_param_key = mtch.int_param_key
				...
				and che.in_cache=0
   join (select number * 3 as mnth from portal_redux.numbers where number between 1 and 16) cal 
...
INSERT INTO portal_redux.[cache_pbcs3_aggr]
				   ([qry_type]
				   ,[date_type]
				   ,[start_date]
				   ,[int_param_key]
...
insert into portal_redux.cache_qry_param_pbcs3
...
if @fl_return_results = 1
        SELECT  pbcs3.[month] as "Month"
            ...
            FROM portal_redux.cache_pbcs3_aggr pbcs3  
            join #cachekeys ck on ck.int_hash_key=pbcs3.int_hash_key
                and pbcs3.qry_id=ck.qry_id
            join portal_redux.ref_lookup_sib_age_grp ref_age on ref_age.cd_sib_age_grp=pbcs3.cd_sib_age_grp
            join portal_redux.ref_lookup_ethnicity_census ref_eth on ref_eth.cd_race_census=pbcs3.cd_race_census
            join portal_redux.ref_lookup_county ref_cnty on ref_cnty.county_cd=pbcs3.cd_county
            join portal_redux.ref_filter_ihs_services ref_ihs on ref_ihs.bin_ihs_svc_cd=pbcs3.bin_ihs_svc_cd
            join portal_redux.ref_filter_reporter_type ref_rpt on ref_rpt.cd_reporter_type=pbcs3.cd_reporter_type
            join portal_redux.ref_filter_access_type ref_acc on ref_acc.cd_access_type=pbcs3.cd_access_type
            join portal_redux.ref_filter_allegation ref_alg on ref_alg.cd_allegation=pbcs3.cd_allegation
            join portal_redux.ref_filter_finding ref_fnd on ref_fnd.cd_finding=pbcs3.cd_finding
