...
CREATE PROCEDURE portal_redux.[sp_ooh_wb_family_settings] (
	@date VARCHAR(3000)
...
AS
EXEC portal_redux.log_query_ooh_wb_family_settings @age_grouping_cd
	,@pk_gender
	,@cd_race_census
...
EXEC portal_redux.build_ooh_wb_family_settings_cache @age_grouping_cd
	,@pk_gender
	,@cd_race_census
	,@initial_cd_placement_setting
	,@longest_cd_placement_setting
...
DECLARE @min_filter_date DATETIME = portal_redux.min_ooh_filter_date(@cd_access_type, @cd_allegation, @cd_finding, @bin_dependency_cd)
DECLARE @cutoff_date DATETIME = (SELECT TOP 1 cutoff_date FROM portal_redux.ref_last_dw_transfer)
...
FROM portal_redux.ooh_wb_family_settings_cache m
...
INNER JOIN portal_redux.vw_dependency_lag dl ON dl.bin_dependency_cd = m.bin_dependency_cd
	AND dl.date_type = m.date_type
	AND m.cohort_entry_date BETWEEN @min_filter_date AND dl.cohort_max_filter_date
INNER JOIN portal_redux.ref_filter_los los ON los.bin_los_cd = m.bin_los_cd
...
INNER JOIN portal_redux.ref_filter_ihs_services ihs ON ihs.bin_ihs_service_cd = m.bin_ihs_service_cd
