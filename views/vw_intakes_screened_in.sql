-- create vw_intakes_screened_in view

-- DROP VIEW portal_redux.vw_intakes_screened_in;
CREATE view [portal_redux].[vw_intakes_screened_in]
as
select
    inf.id_intake_fact,
    inf.id_case,
...
select grp.intake_grouper,
    max(case
        when grp.intk_grp_seq_nbr=1
        then fl_cps_invs
        else 0
        end) intk_grp_fl_cps_invs,
    max(case
        when grp.intk_grp_seq_nbr=1
        then fl_risk_only
        else 0
        end) intk_grp_fl_risk_only,
    max(case
        when grp.intk_grp_seq_nbr=1
        then fl_far
        else 0
    ...
    max( case
        when grp.intk_grp_seq_nbr=1
        then fl_alternate_intervention
        else 0
        end) intk_grp_fl_alternate_intervention,
    max(case
        when grp.intk_grp_seq_nbr=1
        then fl_cfws
        else 0
        end) intk_grp_fl_cfws,
    max(case
        when grp.intk_grp_seq_nbr=1
        then fl_frs
        else 0
    ...
    max(case
        ...
        end) intk_grp_cd_reporter_type,
    max(case
        when grp.intk_grp_seq_nbr=1
        then cd_race_census
        else 0
        end) intk_grp_cd_race_census,
    max(case
        when grp.intk_grp_seq_nbr=1
        then census_hispanic_latino_origin_cd
        else 0
        end) intk_grp_census_hispanic_latino_origin_cd,
    max(case
        when grp.intk_grp_seq_nbr=1
        then cd_sib_age_grp
        else 0
        end) intk_grp_cd_sib_age_grp,
    max(case
        when grp.intk_grp_seq_nbr=1
        then intake_county_cd
        else 0
        end) intk_grp_intake_county_cd,
    max(case
        when grp.intk_grp_seq_nbr=1
        then inv_ass_start
        else null
        end) intk_grp_inv_ass_start,
    max(case
        when grp.intk_grp_seq_nbr=1
        then inv_ass_stop
        else 0
        end) intk_grp_inv_ass_stop,
...
from portal_redux.tbl_intake_grouper grp
join portal_redux.tbl_intakes intk on grp.id_intake_fact=intk.id_intake_fact and intk.cd_final_decision=1
group by grp.intake_grouper ) alg on alg.intake_grouper=grp.intake_grouper
join portal_redux.tbl_intakes inf on inf.id_intake_fact=grp.id_intake_fact;
